...
def add_article_to_batch(parsed_line):
    return [
        ...
        "Article",
        str(uuid5(NAMESPACE_DNS, parsed_line["title"].replace(" ", "_")))
    ]
...
def add_paragraph_to_batch(parsed_line):
    ...
    for paragraph in parsed_line["paragraphs"]:
        add_object = {    
            ...
            "inArticle": [{
                "beacon": "weaviate://localhost/" + str(uuid5(NAMESPACE_DNS, parsed_line["title"].replace(" ", "_")))
            }]
        }
        ...
        return_array.append([
            add_object,
            "Paragraph",
            str(uuid5(NAMESPACE_DNS, parsed_line["title"].replace(" ", "_") + str(paragraph["count"])))
        ])
    return return_array
        ...
        for line in f:
            ...
            if ":" in parsed_line["title"]:
                ...
                for cref in parsed_line["crefs"]:
                    article_uuid = str(uuid5(NAMESPACE_DNS, parsed_line["title"].replace(" ", "_")))
                    link_uuid = str(uuid5(NAMESPACE_DNS, cref))
